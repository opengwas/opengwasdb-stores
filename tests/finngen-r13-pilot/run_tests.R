...
result <- system2(
  "Rscript",
  c("generators/lib/source-formats/finngen-r13-dense/generate.R", paste0("--config=", config_path), "--mode=emit"),
  stdout = TRUE,
  stderr = TRUE
)
...
bad_result <- suppressWarnings(system2(
  "Rscript",
  c("generators/lib/source-formats/finngen-r13-dense/generate.R", paste0("--config=", config_path), "--mode=validate"),
  stdout = TRUE,
  stderr = TRUE
))
...
checksum_result <- suppressWarnings(system2(
  "Rscript",
  c(
    "generators/lib/source-formats/finngen-r13-dense/generate.R",
