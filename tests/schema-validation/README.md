...
opengwasdb#16) instead of `generators/lib/source-formats/gwas-ssf-ragged/generate.R`'s
...
- issue #51 AC3's two named example violations -- an out-of-vocabulary
  ...
  -- each fail `generators/lib/schema_validate.py` directly with a message
...
- the real, already-accepted release bundles this repository tracks
  ...
  against `generators/lib/schema_validate.py` without mutating those bundles'
  own `validation.yaml`.
