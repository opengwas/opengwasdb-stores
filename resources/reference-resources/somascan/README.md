...
coordinate was resolved. SomaLogic's own multi-gene target flag
(`somascan_is_multiple`) stays in `somascan-targets.tsv`; the manifest generator
joins it on this sidecar's `matched_seqid`/`source_seqid` rather than duplicating
it here.
...
```sh
pixi run Rscript resources/generators/somascan/generate-somascan-targets.R
pixi run Rscript resources/generators/somascan/generate-somascan-analysis-targets.R \
  --pubmed-id=29875488 \
  --output=resources/reference-resources/somascan/sun-2018-analysis-targets.tsv
```
