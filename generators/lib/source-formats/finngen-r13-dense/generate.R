...
script_version <- function(root) {
  paste0("sha256:", sha256_file(path_abs(root, "generators/lib/source-formats/finngen-r13-dense/generate.R")))
}
...
emit_bundle <- function(cfg, root, config_path) {
  ...
  write_yaml_file(list(
    ...
    accepted_at = NULL,
    generator = list(
      name = "generators/lib/source-formats/finngen-r13-dense/generate.R",
      version = script_version(root),
      command = paste("Rscript generators/lib/source-formats/finngen-r13-dense/generate.R", paste0("--config=", config_path), "--mode=emit")
    ),
  ...
  write_yaml_file(list(
    status = "candidate",
    validated_at = NULL,
    validator = list(name = "generators/lib/source-formats/finngen-r13-dense/generate.R", version = NULL),
    checks = list(
      selection = "passed",
...
root <- repo_root()
source(path_abs(root, "generators/lib/metadata_resolvers/finngen_manifest.R"))
source(path_abs(root, "generators/lib/build_environment.R"))
source(path_abs(root, "generators/lib/schema_validate.R"))
source(path_abs(root, "generators/lib/finngen_r13_pilot.R"))
